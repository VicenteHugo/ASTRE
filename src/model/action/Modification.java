...
public class Modification extends Action
{
	...
	public Modification (Affectations a) 
	{
		...
		this.requetes = "UPDATE Affectation SET libCatHeur = ?, nbSem = ?, nbGroupe = ?, commentaire = ? WHERE intNom = ? AND intPrenom = ? AND codeMod = ?";
		this.info = new ArrayList<>(List.of(a.getCategorieHeures(), a.getNbSemaine(), a.getNbGroupe(), a.getCommentaire(),
                                            a.getIntervenant().getNomIntervenant(), a.getIntervenant().getPrenomIntervenant(), a.getModule().getCode()));
	}

	public Modification (Module m) 
	{ 
		this.requetes = "UPDATE Modules SET semMod = ?, typeMod = ?, libCourtMod = ?, libLongMod = ?, validMod = ?, nbHeurPonc = ? WHERE codeMod = ?" ;

		String code = m.getCode();
		this.info = new ArrayList<>(List.of(m.getSemestres(), m.getListCategorieHeure(), m.getLibCourt(), m.getLibLong(), m.isValide(), m.getHeurePonctuel(),code));
	}
	...
	{ 
		...
		this.info = new ArrayList<>(List.of(inter.getServices(), inter.getMaxHeures(),inter.getCategorieIntervenant(), nom, prenom));
	}
	...
	public Modification (CategorieIntervenant cat) 
	{
		...
		this.info.addAll(List.of(cat.getLibCatInt(), cat.getCoefCatInt(), cat.getHeureMinCatInt(), cat.getHeureMaxCatInt(), code));
	}
